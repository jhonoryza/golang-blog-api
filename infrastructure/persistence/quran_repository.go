...
func (r *QuranRepository) FindAllSurah(ctx context.Context) *[]entities.Quran {
	...
	var surah []entities.Quran
	for rows.Next() {
		var quran entities.Quran
...
func (r *QuranRepository) FindAllAyahFromSurah(ctx context.Context) *[]entities.QuranVerse {
	search := ctx.Value("search").(string)
	sortBy := ctx.Value("sortBy").(string)
	sortDir := ctx.Value("sortDir").(string)
	...
	var ayah []entities.QuranVerse
	for rows.Next() {
		var verse entities.QuranVerse
