...
func (r *ToolRepository) FindAll(ctx context.Context) *[]entities.Tool {
...
func (r *ToolRepository) FindOneById(toolId string) *entities.Tool {
	query := `
		select id, name, tools.desc, link, is_published, type, created_at, updated_at
		from tools
		where id = $1
	`

	var tool entities.Tool
	err := r.DB.QueryRow(query, toolId).Scan(&tool.ID, &tool.Name, &tool.Description, &tool.Link, &tool.IsPublished, &tool.Type, &tool.CreatedAt, &tool.UpdatedAt)
	if err != nil {
		return nil
	}
...
func (r *ToolRepository) Create(req request.CreateToolRequest) (*entities.Tool, error) {
	query := `
...
func (r *ToolRepository) Update(req request.UpdateToolRequest, toolId string) (*entities.Tool, error) {
	query := `
		update tools
